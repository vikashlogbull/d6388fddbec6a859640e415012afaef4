...
class DeploySiteController extends Phalcon\DI\Injectable {
    ...
    public function Deploy()
    {
        ...
        try {
			...
			if(trim(strtolower($target_environmentname)) == "test"){
				$target_sitedomainname = "test_".$source_sitedomainname;
			}
			else {
				$target_sitedomainname = strtolower($target_environmentname).substr($source_sitedomainname,4);
			}
			...
			//echo "SELECT A1.id AS database_server_id, A2.serverid AS dbid, A1.db_external_hostname AS dbconnection_string, A1.db_port AS dbport, A1.db_server_username AS dbusername, A1.db_server_password AS dbpassword FROM databaseserver A1 JOIN server A2 on A1.serverid = A2.serverid JOIN xrefServerCompany A3 ON A2.serverid = A3.serverid  WHERE A1.is_primary_db_server = 1 AND A1.status = 'active' AND A2.serverstatus = 'active' and  A3.backendcompanyid = $companyid AND A2.environmentid = $source_environmentid: LIMIT 1 ";
			//die;	
			...
			//print_r($serverips); 			 
			//echo  'http://IP/API2/DeploySite.php?apikey='.$apikey.'&sitename='.$sitename.'&companyid='.$companyid.'&giturl='.$giturl.'&gitbranch='.$gitbranch.'&siteurl='.$target_sitedomainname.'&environment='.$target_environmentname.'&database_url='.$SourceDatabaseLink.'&drup_dbname='.$drup_dbname.'&db_ip='.$dbconnection_string.'&dbuser='.$dbusername.'&password='.$dbpassword.'&NewSite='.$NewSite.'&source_serverip='.$source_serverip.'&source_server_baseurl='.$source_server_baseurl;
			//die;
	...
	public function IsSiteDeployed($siteid,$environmentid){
		...
		try {
			...
			$getListenerRes = $clientlistener->request('GET','/API2/IsSiteFolderExist.php',['query' => ['drupalfolderpath' => $sitexrefdata["drupalfolderpath"]]])->getBody()->getContents();
			if(!$getListenerRes){
				return false;
			}
		}
	...
	function exportDatabase($siteid,$source_environmentid){
		...
		$serverPath = $serverip."/API2/ExportDBToSQL.php?drupalfolderpath=".$drupalfolderpath;
		...
		return json_encode(array('Status' => 'Success', 'Data' => $serverip."/API2/".md5($drupalfolderpath).".sql"));
	}
}