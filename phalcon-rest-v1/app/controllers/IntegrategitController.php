...
class IntegrategitController extends Phalcon\DI\Injectable {
    ...
    public function CreateRepoForSite($RepoAccountID, $companyid, $companyname, $siteid, $repopath) {
        ...
        // Checkin initial data for get commits
        $username = 'awsashish';
        $useremail = 'ashish@logbullit.com';
        exec('git init');
        exec('git config user.name '.$username);
        exec('git config user.email '.$useremail);
        exec('echo "# 66a4976c067a8e9ddc25f52df1afd467" >> EVOLVER_README.md');
        exec('git add *');
        exec('git commit -m "Commits while private repo creation" ');
        exec('git remote rm origin');
        exec('git remote add origin '.$RepoGitPUrl);
        exec('git remote -v');
    ...
    public function create() {
        ...
        if (empty($clientgit) && empty($clientuser) && empty($clienttoken)) {
            ...
            if ($gitdecode->{'Status'} == "Success") {
                ...
                $git_branch_id = $gitbranch->setGitBranch($privategitid, 'master', '1', '1', 'TRUE');
                error_log("GitBranchID: ".$git_branch_id);
 
                $updategitonsiteenv = new siteenvironment();
                $environmentid = 1;
                $gitupdate = $updategitonsiteenv->updateGitData($siteid, $environmentid, $privategitid, $git_branch_id);
            }else {
                $this->response = Array(
                      "Status" => "Error",
                      "Code" => "400",
	    ...
	    header('Content-Type: application/json');
            ...
            if ($gitdecode->{'Status'} == "Success") {
                ...
                $repo_complete_url = $gitdecode->{'repo_complete_url'};
                $privategitid = $gitdecode->{'privategitid'};
                error_log("PrivateGitID : ".$privategitid);
                error_log("Private_Git_Info".$RepoGitPUrl);
            }else {
                $this->response = Array(
                      "Status" => "Error",
            ...
            $gitObj = new git();
            $publicgitid = $gitObj->setGit($companyid, 'null', $clientgit, 'null', $clientuser, 'null', 'No', $clienttoken, 'public', $repo_name,'True', 'null', $repopath);
	    ...
	    $gitbranchobj = new gitbranch();
            $privategitbranchid = $gitbranchobj->setGitBranch($privategitid, $branchsuffix, $pubgitbranchid, $activestatus, $is_private_master);
            error_log("PublicGitBranch: ".$pubgitbranchid."_____ PrivateGitBranchID".$privategitbranchid);
            // Update the contected to detail.
            $gitbranchobj = new gitbranch();
            $updategitconnection = $gitbranchobj->updateGitBranchReference($pubgitbranchid, $privategitbranchid);
		...
		if (is_null($environmentid)){
            ...
            error_log(json_encode($publichookdata));
