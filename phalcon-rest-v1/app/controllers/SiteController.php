...
class SiteController extends Phalcon\DI\Injectable {
	...
	protected function postgresDBConnection(){
		...
		$host        = "host=db.stratosys.tech";
		$port        = "port=5432";
		$dbname      = "dbname=stratopaas";
...
EOF;
	...
	public function GetIPAddressOfSites(){
        ...
        try {
            ...
            $environmentid 	= $this->app->request->get("environmentid");
            $siteid 		= $this->app->request->get("siteid");
            $subsiteid 		= $this->app->request->get("subsiteid");
			...
			if($environmentid == ""){
				return $this->app->response
					->setStatusCode(401, "Unathorized") 
					->setJsonContent(array('status' => 'ERROR', 'MsgDesc' => 'Environment ID is a mandatory field.'));
			}

			if($siteid == "" && $subsiteid == ""){
				return $this->app->response
					->setStatusCode(401, "Unathorized") 
					->setJsonContent(array('status' => 'ERROR', 'MsgDesc' => 'Site ID Or Subsite Id is a mandatory field.'));
			}			
...
EOF;
			...
			if(pg_num_rows($serverIPResult)>0){
				...
				if(curl_getinfo($ch, CURLINFO_HTTP_CODE) != 404) {
					if($return == ""){
						...
						$this->app->response
							->setStatusCode($return->status_code, $return->title)
							->setJsonContent($return->json_content);
					}
				}
				else {
					$this->app->response
	...
	public function ImportDBBySQLFileURL() {
        ...
        try {
            ...
            $environmentid	 	=  $this->app->request->get("environmentid");
            $siteid 			=  $this->app->request->get("siteid");
            $database_url 		=  $this->app->request->get("database_url");
			...
			if($environmentid == "" || $siteid == "" || $database_url == ""){
				return $this->app->response
					->setStatusCode(401, "Unathorized")
			...
			$database_url_array = explode(".",$database_url);
			$database_file_extension = end($database_url_array);
			if(strtolower($database_file_extension) != "sql"){
				return $this->app->response
					->setStatusCode(401, "Unathorized")
					->setJsonContent(array('status' => 'ERROR', 'MsgDesc' => 'Failure! Input file is not a sql file.'));
			}
...
EOF;
			...
			if(curl_getinfo($ch, CURLINFO_HTTP_CODE) == 404) {
				return $this->app->response
					->setStatusCode(401, "Unathorized")
					->setJsonContent(array('status' => 'ERROR', 'MsgDesc' => 'Unexpected error on the server, where site is deployed'));
			}
			curl_close($ch);
			if($dbImported == 1){
				$this->app->response
	...
	public function ImportFiles() {
        ...
        try {
            ...
            $environmentid	 	=  $this->app->request->get("environmentid");
            $siteid 			=  $this->app->request->get("siteid");
            $archivefileurl 	=  $this->app->request->get("archivefileurl");
			...
			if($environmentid == "" || $siteid == "" || $archivefileurl == ""){
				return $this->app->response
					->setStatusCode(401, "Unathorized")
...
EOF;
	...
	public function ExportDBToS3() {
        ...
        try {
            ...
            $siteid 		= $this->app->request->get("siteid");
            $subsiteid 		= $this->app->request->get("subsiteid");
            $is_backup 		= $this->app->request->get("is_backup");
            ...
            if (!isset($is_backup) || empty($is_backup) || $is_backup == 0) {
                $is_backup = 0;
            }
			else {
                $is_backup = 1;
...
EOF;
			...
			$serverPath = "http://".$serverIP."/API/ExportDBToS3.php?environmentid=".$environmentid."&siteid=".$siteid."&subsiteid=".$subsiteid."&is_backup=".$is_backup;
			$ch = curl_init();
			curl_setopt($ch, CURLOPT_URL, $serverPath); 
			curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1); 
			$result = curl_exec($ch);
...
EOF;
			...
			$serverPath = "http://".$serverIP."/API/ExportFilesToS3.php?environmentid=".$environmentid."&siteid=".$siteid."&subsiteid=".$subsiteid."&is_backup=".$is_backup;
	...
	public function GetListOfHistoricalDBExports() {
        ...
        try {
			...
			if($siteid != ""){
				$SQL =<<<EOF
				SELECT id,uploaded_timestamp,filesize from epm_site_export_details WHERE siteid = $siteid AND environmentid=$environmentid AND file_type='1' AND is_backup='0' ORDER BY id DESC;
EOF;
			}
			else {
				$SQL =<<<EOF
				SELECT id,uploaded_timestamp,filesize from epm_site_export_details WHERE subsiteid = $subsiteid AND environmentid=$environmentid AND file_type='1' AND is_backup='0' ORDER BY id DESC;
EOF;
			}
	...
	public function GetListOfHistoricalFileExports() {
        ...
        try {
			...
			if($siteid != ""){
				$SQL =<<<EOF
				SELECT id,uploaded_timestamp,filesize from epm_site_export_details WHERE siteid = $siteid AND environmentid=$environmentid AND file_type='0' AND is_backup='0' ORDER BY id DESC;
EOF;
			}
			else {
				$SQL =<<<EOF
				SELECT id,uploaded_timestamp,filesize from epm_site_export_details WHERE subsiteid = $subsiteid AND environmentid=$environmentid AND file_type='0' AND is_backup='0' ORDER BY id DESC;
EOF;
			}
	...
	public function GetPresignedURL(){
		...
		try {
			// Receiving get parameters 
            $apikey 		= $this->app->request->get("apikey");
            $unique_id 		= $this->app->request->get("id");
			...
			$SQL =<<<EOF
			SELECT * FROM epm_site_export_details WHERE id = $unique_id;
EOF;
			$Result 	= pg_query($db, $SQL);
			if(pg_num_rows($Result) == 0){
				return $this->app->response
