...
class DockerController extends Phalcon\DI\Injectable {
    ...
    public function create()
    {
        try {
            ...
            # Get Bucket Name
            $BucketName = $objcompany->getBucketName($frontendcompanyid);
            if (is_null($BucketName)) {
                $this->app->response
                        ->setStatusCode(404, "Not Found")
                        ->setJsonContent(array('status' => 'ERROR', 'data' => 'Unable to get AWS Bucket Name from DB!'));
                return $this->app->response;
            }
			...
			if ($InstallType == 'FRESH' && $drupal_distribution != 'null'){
				...
				// Get DB sql file from given public git url
			}
...
EOF;
			...
			if (!file_exists(".htaccess")) {
				exec('cp '.$doc_root.'/source_files/.htaccess .');
			}
			...
			$privateGitFolder = $siteid."_".strtoupper($environmentname);
			...
			if($file_public_path != "" && @is_dir($file_public_path) && !$this->is_dir_empty($file_public_path)){
				...
				chdir($doc_root.'/'.$PrivateRepoDirectory);
				$PublicFilesZipName = trim($sitename).'_'.trim($environmentname).'_'.date("Y-m-d").'_'.date("h:i:s").'_'.date_default_timezone_get().'_public_files.zip';

				if(!$this->CreateZip($file_public_path,$PublicFilesZipName)){
					error_log("Failed! ZIP can not be created.");
				}

				$AWSURI  = $siteid."/".$environmentname."/drup_file_archive/".$PublicFilesZipName;
				
				$AWSLinkArray = $this->UploadFilesOnS3Bucket($S3BucketDetails,$BucketName,$PublicFilesZipName, $AWSURI);
				...
				$publicFileExportID = $objExport->addRecord($siteid,$subsiteid,$environmentid,'public_files',$file_path,$uploaded_timestamp,$filesize,$BucketName,$AWSURI);
				
				if(!$publicFileExportID){
					error_log("Failed! Error when inserting AWS public file details into the database.");
				}
			}
			if(!$publicFileExportID) $publicFileExportID = 0;
			

			// Fetching private file path
			...
			if($file_private_path !="" && is_dir($file_private_path) && !$this->is_dir_empty($file_private_path)){
				...
				$AWSURI  = $siteid."/".$environmentname."/drup_file_archive/".$PrivateFilesZipName;
				...
				$subsiteid				= NULL;
				$objExport 				= new siteExportDetails();
				$privateFileExportID	= $objExport->addRecord($siteid,$subsiteid,$environmentid,'private_files',$file_path,$uploaded_timestamp,$filesize,$BucketName,$AWSURI);
				
				if(!$privateFileExportID){
					error_log("Failed! Error when inserting AWS private File details into the database");
				}
			}
			if(!$privateFileExportID) $privateFileExportID = 0;
			
			//echo $publicFileExportID;
			//echo "===============";
			//echo $privateFileExportID;
			//echo "===============";
			//die;
			...
			if ($InstallType == 'FRESH' && $drupal_distribution != 'null'){
                ...
                foreach($serverips as $serverip)
                {
                    ...
                    try {
						...
						$pipelinename = $siteid."-".strtoupper($environmentname);
						...
						$dockername = $siteid."-".strtoupper($environmentname)."1";
						...
						$createPipeline = $this->createPipeline($siteid,$pipelinename,$PrivateRepoGitUrl,$PrivateClientBranch,$serverip,$serverport,$privateGitFolder,$S3BucketDetails);
				...
				echo "publicFileExportID : " . $publicFileExportID;
				echo "==============";
				echo "privateFileExportID : " . $privateFileExportID;
				echo "==============";
				echo "gitfoldername : " . $privateGitFolder;
				echo "==============";
            }
	...
	public function createPipeline($SiteID,$PipelineName,$GitURL,$GitBranch,$ServerIP,$ServerPort,$PrivateGitFolder,$S3BucketDetails){
