...
class DeploySiteController extends Phalcon\DI\Injectable {
    ...
    public function Deploy()
    {
        try {
			...
			$gitid					= trim($sitexrefdata["gitid"]);
			$git_branch_id			= trim($sitexrefdata["git_branch_id"]);
			
			
            # Get site backend company id
            $objgitbranch = new gitbranch();
            $gitbranchdata = $objgitbranch->getGitBranchData($git_branch_id);
			...
			try {
				...
				if (!(($commits = $client->request('GET', 'github/getallcommits/'.$fcompanyid.'/'.$privategitid.'/'.$source_git_branch)->getBody()) &&
			...
			if ($xrefid) {				
				$updateXREFData = $objsiteenv->updateSite(trim($xrefid), trim($siteid), trim($target_environmentid), trim($drupalfolderpath), trim($serverid), trim($target_sitedomainname), trim($dbconnection_string), trim($drup_dbname), trim($username), trim($password), trim($git_branch_id), trim($database_uses_global_password), trim($database_id), trim($gitid), trim($site_status));
			}
			else {
				$xrefid = $objsiteenv->setSite(trim($siteid), trim($target_environmentid), trim($drupalfolderpath), trim($serverid), trim($target_sitedomainname), trim($drup_dbname), trim($dbconnection_string), trim($username), trim($password), trim($git_branch_id), trim($database_uses_global_password), trim($database_id), trim($gitid), trim($site_status));
			}

			//print_r(array('companyid' => $companyid, 'siteid' => $siteid, 'sitenameforgit' => $sitename, 'source_environmentid' => $source_environmentid, 'target_environmentid' => $target_environmentid, 'git_id' => $gitid, 'git_branch_id' => $git_branch_id));
			//die;
			
			try {
				$client = new GuzzleHttp\Client(['base_uri' => 'http://api.evolverinc.tech']);
				$gitresponse = $client->request('GET','/phalcon-rest-v1/integrategit/deploy', ['query' => ['companyid' => $companyid, 'siteid' => $siteid, 'sitenameforgit' => $sitename, 'source_environmentid' => $source_environmentid, 'target_environmentid' => $target_environmentid, 'git_id' => $gitid, 'git_branch_id' => $git_branch_id, 'gitsha' => $gitsha]])->getBody()->getContents();
			}
			catch(Exception $e){
				$this->app->response
						->setStatusCode(404, "Not Found")
			...
			//print_r($serverips); 			 
			//echo  'http://'.$serverip.'/API/DeploySite.php?apikey='.$apikey.'&sitename='.$sitename.'&companyid='.$companyid.'&giturl='.$giturl.'&gitbranch='.$gitbranch.'&siteurl='.$target_sitedomainname.'&environment='.$target_environmentname.'&database_url='.$SourceDatabaseLink.'&drup_dbname='.$drup_dbname.'&db_ip='.$dbconnection_string.'&dbuser='.$dbusername.'&password='.$dbpassword.'&NewSite='.$NewSite;
			//die;
			...
			foreach($serverips as $serverip)
			{
				try {
					...
					$getListenerRes = $clientlistener->request('GET',
						'/API/DeploySite.php',
						['query' => ['apikey' => $apikey, 'sitename' => $sitename, 'companyid' => $companyid, 'giturl' => $giturl, 'gitbranch' => $gitbranch, 'siteurl' => $target_sitedomainname, 'environment' => $target_environmentname, 'database_url' => $SourceDatabaseLink, 'drup_dbname' => $drup_dbname, 'db_ip' => $dbconnection_string, 'dbuser' => $dbusername, 'password' => $dbpassword, 'NewSite' => $NewSite]])->getBody()->getContents();
				}
				catch(Exception $e){
					$this->app->response
							->setStatusCode(404, "Not Found")
			...
			$dbsecondryuser = $client->request('GET','/phalcon-rest-v1/site/addusertodb/',['query' => ['apikey'=> $apikey, 'environmentid' => $target_environmentid, 'siteid' => $siteid]])->getBody()->getContents();
