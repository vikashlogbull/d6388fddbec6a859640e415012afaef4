...
class GithubController extends Controller {
	...
	public function deploysAction() {

		if ($this->request->isGet() &&
			($env = $this->request->get("env", "string")) && 
...
*/
	...
	public function makeRepoBranch($company_id,$site_id,$git_id,$branch_name,$sha) {
		...
		if (($branch_name_arr = explode('_',$branch_name)) &&
			...
			($branch_type == 'master' || $branch_type == 'test' || $branch_type == 'live') &&
		    ...
		    try {
				if (($res = $this->modelsManager->executeQuery("
				    ...
				    if (isset($res2[0])) {
						...
						$this->response['MsgDescription']	= 'The branch is already exists';
					} else {
						...
						try {
							if (is_array($ref = $client->api('git')->references()->create(
								...
								try {
									...
									$git_branch->git_id = $git_id;
									$git_branch->branch_name = $branch_name;
					    
									if ($git_branch->save() &&
										($res = $this->modelsManager->executeQuery("
											SELECT environmentid AS env_id
											FROM environment
											WHERE LOWER(environmentname) = :env:
										", array(
		   	   	   	   	   	   	   	   		'env'=> $env,
										))) &&
		                                ...
		                                $xref_site_env = new xrefSiteEnvironment;
										$xref_site_env->siteid = $site_id;
										$xref_site_env->environmentid = $env_id;
										$xref_site_env->gitid = $git_id;
										$xref_site_env->git_branch_id = $git_branch_id;
					    
										if ($xref_site_env->save()) {
					    
										    $xref_id =  $xref_site_env->xrefid;
										    ...
										    $err_msg = "Cannot save the new branch!";
                                        }
									} else {
                                        $err_msg = "Cannot save the new branch";
                                    }
				...
				case 'push':
					...
					if (($res = $this->modelsManager->executeQuery("
							SELECT g.git_id,gb.id AS branch_id,g.git_username,g.github_token,
								g.repo_name,g.repo_path,gb.connected_to
							FROM Git g
						...
						if ($connected_to = &$res[0]->connected_to) {
						    // our private repo
error_log("A Private Repo Push: git_id=".var_export($git_id,true).", git_branch_id=".var_export($git_branch_id,true));

							$local_repo_path = &$res[0]->repo_path;
	
							$cmd = 'cd '.$local_repo_path.'\\'.$env.
								' && git pull origin '.$env;
							$ret = exec($cmd);
error_log("pr push cmd=".var_export($cmd,true));
error_log("pr push res=".var_export($res,true));
...
error_log("Original repo push: git_id=".var_export($git_id,true).", git_branch_id=".var_export($git_branch_id,true));
							...
							if (($github_token = trim($res[0]->github_token)) &&
								...
								$ret = exec($cmd);
error_log("or push cmd=".var_export($cmd,true));
error_log("or push res=".var_export($res,true));
							}
						}
					}
					break;
...
/*
	public function makeDeploy() {
		...
		if ($this->request->isPost() &&
			...
			($env_id 	= $this->request->getPost('env_id')) &&

			($res = $this->modelsManager->executeQuery("
				SELECT g.git_username AS repo_owner,g.repo_name,g.git_url,g.git_ssh,g.github_token AS repo_token,
				FROM epm_sites s
					LEFT JOIN epm_xref_site_environment se ON se.siteid=s.siteid
					LEFT JOIN epm_xref_git_sites gs ON se.gitid = gs.gitid AND se.sitesid = gs.sitesid
			...
			$res[0]->repo_owner && $res[0]->repo_name && $res[0]->repo_token &&
			($client = new \Github\Client(
				new \Github\HttpClient\CachedHttpClient(array('cache_dir' => '/tmp/github-api-cache'))))
		) {
			$client->authenticate($this->repo_token,Github\Client::AUTH_HTTP_TOKEN);
			$_params = array('ref' => $env_id,'environment' => $env_id,'auto_merge' => true);

			if (is_array($deploy = $client->api('deployment')->create(
				$res[0]->repo_owner, $res[0]->repo_name, $_params))
			) {
				$this->response = array(
		        	"Status"		=> "Success",
	    	    	"MsgCode"		=> "100",
