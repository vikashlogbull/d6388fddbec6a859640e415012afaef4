...
class EpmsitesController extends Phalcon\DI\Injectable {
    ...
    public function create()
    {
        try {
            ...
            if (($resdb = $this->modelsManager->executeQuery("

                SELECT A1.db_external_hostname AS dbconnection_string 
                FROM databaseserver A1
                JOIN server A2 on A1.serverid = A2.serverid
                JOIN xrefServerCompany A3 ON A2.serverid = A3.serverid 
                WHERE A1.status = 'active' AND A2.serverstatus = 'active' and 
                A3.backendcompanyid = $companyid AND A2.environmentid = $environmentid")) && isset($resdb[0]) 

            //&& ($dbconnection_string = &trim($resdb[0]->dbconnection_string))) {
            && ($dbconnection_string = &trim($resdb[0]->dbconnection_string))) {
                $dbserverips = $dbconnection_string;
            }
