    ...
    public function Deploy()
    {
        ...
        try {
			...
			try {
				...
				error_log("Calling integrategit");
				$gitresponse = $clientgit->request('GET','/phalcon-rest-v1/integrategit/deploy', ['query' => ['companyid' => $companyid, 'siteid' => $siteid, 'sitenameforgit' => $sitename, 'source_environmentid' => $source_environmentid, 'target_environmentid' => $target_environmentid, 'site_private_git_id' => $site_private_git_id, 'source_git_branch_id' => $source_git_branch_id, 'site_private_git_sha_key' => $site_private_git_sha_key]]);
				error_log(json_encode($gitresponse));
			}
			catch(Exception $e){
				$this->app->response
						->setStatusCode(404, "Not Found")
