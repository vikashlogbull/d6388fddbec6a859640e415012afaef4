...
class DeploySiteController extends Phalcon\DI\Injectable {
    ...
    public function Deploy()
    {
        try {
			...
			//print_r($serverips); 			 
			//echo  'http://'.$serverip.'/API/DeploySite.php?apikey='.$apikey.'&sitename='.$sitename.'&companyid='.$companyid.'&giturl='.$giturl.'&gitbranch='.$gitbranch.'&siteurl='.$target_sitedomainname.'&environment='.$target_environmentname.'&database_url='.$SourceDatabaseLink.'&drup_dbname='.$drup_dbname.'&db_ip='.$dbconnection_string.'&dbuser='.$dbusername.'&password='.$dbpassword.'&NewSite='.$NewSite.'&source_serverip='.$source_serverip.'&source_server_baseurl='.$source_server_baseurl;
			//die;
